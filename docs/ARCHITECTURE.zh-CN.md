...
sidepanel/init.ts  ─ mount Preact 组件，缓存 DOM 引用，
                     加载设置，打开 chrome.runtime.connect("image-harvest-ui")，
                     调用 loadCurrentTab()
        │
        ▼ （异步）
...
'image-harvest-ui'})` 打开的长连接 ports。这就是 `IMAGES_DISCOVERED`
  能不带 ack 仪式地推到面板的原理。
...
| `referral.ts`            | `getReferralStatus`、`copyReferralLink`、`shareReferralLink`、`claimReferral`、`generateFingerprint`、`matchReferral`                                                                        | 推荐/邀请系统 —— 链接分享、指纹匹配、奖励认领。内建防自引用保护。v1.0.6 新增。                                                |
| `remote-config.ts`       | `fetchRemoteConfig`、`getCachedConfig`、`syncRemoteConfig`                                                                                                                                   | 三级缓存（内存 → chrome.storage → 网络），1 小时 TTL，8 秒超时。`globalThis.__remoteConfig` 供同步读取。v1.0.6 新增。         |
| `feature-quota.ts`       | `checkFeatureQuota`、`incrementFeatureUsage`、`getAllFeatureQuotas`                                                                                                                          | 月/日软配额系统，替代硬 Pro 门控。周期边界自动重置。v1.0.6 新增。                                                             |
| `eagle-free-quota.ts`    | _（空占位文件）_                                                                                                                                                                             | Eagle 月度配额在 v1.0.6 中移除；文件保留为占位。`export {}`。                                                                 |
...
`'image-harvest-ui'` port 的 `port.postMessage`（广播）。下面的表格
记录了每条接线契约。
...
| 消息                | 形态                          | 行为                                                                           |
| ------------------- | ----------------------------- | ------------------------------------------------------------------------------ |
| `IMAGES_DISCOVERED` | `{type, images: ImageItem[]}` | 后台再广播给所有连接到 `image-harvest-ui` port 的端（即每个打开的面板/弹窗）。 |
...
2. 在 `shared/types.ts` 加请求 / 响应类型：
   ```ts
   export interface GetTabFaviconRequest {
     tabId?: number;
   }
   export interface GetTabFaviconResponse {
     success: boolean;
     faviconUrl?: string;
     error?: string;
   }
