...
- **Contact**: `coderkyriewen@gmail.com`
...
## 3. What is sent off your device — *and only with reason*
...
**When:** when you click *Activate* with a license key, when you click
*Deactivate*, and once every 24 hours via a `chrome.alarms` background
...
- A random per-install `instanceId` (so we can enforce one active key
  per machine; this is *not* tied to you in any way).
...
**What we send to *our* servers:** nothing. Reverse search bypasses our
backend entirely.
...
- ❌ Geolocation beyond a coarse country derived from request IP, which
  is discarded *before* the event reaches our database.
...
where applicable law gives them. Email `coderkyriewen@gmail.com` with
...
> Note: because telemetry batches are *not* tied to a real-world
...
We will *never* expand telemetry beyond the whitelist in
`shared/telemetry-events.ts` without bumping the schema version *and*
re-prompting.
...
| Image bytes never leave your machine | `background/reverse-search.ts > fetchImageData` (we proxy *to* the panel, not *to* a server) |
...
Privacy questions: `coderkyriewen@gmail.com` with subject prefix
`[PRIVACY]`.
