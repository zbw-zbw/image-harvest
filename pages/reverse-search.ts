// Standalone tab opened by the extension to perform a reverse-image search.
// Receives ?engine=...&imageUrl=... query params, downloads the image via
// the background service worker, then submits it to the chosen engine
// either as a multipart form upload or via a URL fallback.

interface ReverseSearchUploadResult {
  success?: boolean;
  redirectUrl?: string;
}

interface FetchImageDataResult {
  success?: boolean;
  dataUrl?: string;
}
...
(async function (): Promise<void> {
  ...
  try {
    ...
    if (statusP) statusP.textContent = `Downloading image for ${engine} search...`;
    ...
    if (statusP) statusP.textContent = `Submitting image to ${engine}...`;
  ...
  async function submitYandex(imageDataUrl: string, imageUrl: string): Promise<void> {
  ...
  async function submitBaidu(imageDataUrl: string, imageUrl: string): Promise<void> {
    // Baidu requires opening their page and injecting a script to simulate
    // file upload (their API rejects direct fetch requests)
    try {
      const statusP = statusEl?.querySelector('p');
      if (statusP) statusP.textContent = 'Opening Baidu image search...';
      const result: ReverseSearchUploadResult | undefined = await chrome.runtime.sendMessage({
        type: 'REVERSE_SEARCH_UPLOAD',
        engine: 'baidu',
        imageDataUrl: imageDataUrl,
  ...
  function createFileInput(fieldName: string, blob: Blob, fileName: string): HTMLInputElement {
  ...
  function fallbackUrlSearch(engine: string, imageUrl: string): void {
