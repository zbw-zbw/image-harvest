...
// Use var instead of let to allow re-injection without "already declared" errors
...
// After an extension reload / update the old content script stays alive
// but chrome.runtime becomes unusable — any property access throws
...
async function handleMessage(message, sendResponse) {
  switch (message.type) {
    ...
    case MESSAGE_TYPES.EXTRACT_IMAGES:
      const images = await extractImages({ skipIframes: message.skipIframes });
      sendResponse({ success: true, images });
      break;
      
    case MESSAGE_TYPES.START_LIVE_MONITOR:
      startLiveMonitoring(message.config);
...
async function extractImages(options = {}) {
  // Allow new extraction even if a previous one is still running (e.g. when
  // the popup was closed mid-scan and reopened). The previous async operations
  // will complete harmlessly but their results are discarded.
  isExtracting = true;
  seenUrls.clear();
  ...
  try {
    ...
    await extractFromStylesheets(images);
    ...
    // Skip when searchAllFrames is enabled — sub-frames are queried independently
    // by background.js, so main frame should not also extract via DOM access
    // to avoid duplicate/conflicting results that can cause images to be lost.
...
// Extract <img> tags
async function extractImgTags(images) {
  ...
  for (const img of imgElements) {
    try {
      ...
      // Collect all candidate URLs from this <img>: src, currentSrc, srcset, and lazy-load attrs
...
async function extractBackgroundImages(images) {
...
async function extractPictureSources(images) {
...
// Extract from stylesheets (limited due to CORS)
async function extractFromStylesheets(images) {
  try {
      ...
      try {
        ...
        for (const rule of rules) {
          if (rule.style && rule.style.backgroundImage) {
            const urls = extractBackgroundUrls(rule.style.backgroundImage);
            for (const url of urls) {
              if (!url || isGradient(url)) continue;
              
              if (isDataUri(url)) {
                // Data URI in stylesheets — skip, they'll be caught by getComputedStyle
                continue;
              }
