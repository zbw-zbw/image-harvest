...
- **无分析追踪** —— 我们不收集使用分析、遥测数据或任何形式的追踪信息
- **无第三方服务** —— 不接入 Google Analytics、不接入 Mixpanel、无广告网络
...
**最新版本**：`v1.0.12` —— 📊 修复 14 个匿名产品事件被后端默默丢弃的问题、🎯 定价页来源归因（不采集额外个人数据）、✅ 支付漏斗真实化、🧪 新增 51 个扩展测试（共 1518）+ 后端首套自动化测试（61 用例）与 GitHub Actions CI。完整列表见 [CHANGELOG.md](./CHANGELOG.md)。
