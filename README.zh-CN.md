...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/隐私优先-无分析追踪-success" alt="隐私优先 · 无分析追踪">
...
- 🛡️ **[隐私政策](./docs/PRIVACY.zh-CN.md)** —— 数据处理、隐私细节、用户控制（[English](./docs/PRIVACY.md)）
...
| 单张下载                           | ✅ 完整           | ✅ 完整             |
| 侧边栏 / 弹窗双模式                | ✅ 完整           | ✅ 完整             |
| 批量 ZIP 下载                      | ⚡ 30 张/次       | ✅ 无限制           |
| 批量复制 URL                       | ⚡ 10 个/次       | ✅ 无限制           |
| Eagle 导出                         | ⚡ 10 张/次       | ✅ 无限制           |
| 批量删除                           | ⚡ 15 张/次       | ✅ 无限制           |
| 批量收藏                           | ⚡ 15 张/次       | ✅ 无限制           |
| 批量高亮                           | ⚡ 3 次/天        | ✅ 无限制           |
| 格式转换                           | ⚡ 5 次/月        | ✅ PNG / JPG / WebP |
| 自定义命名模板                     | ⚡ 仅默认模板     | ✅ 完整模板变量     |
| 页面高亮                           | ⚡ 仅单张         | ✅ 批量 + 自动滚动  |
| 智能分组                           | ⚡ 无 / 按格式    | ✅ 全部 5 种        |
| 实时监控                           | ⚡ 1 次/月        | ✅ 实时监听         |
| 相似图片检测                       | ⚡ 3 次/月        | ✅ 基于 pHash       |
| AI 标签                            | ⚡ 10 次/月       | ✅ 无限制           |
| 图片收藏                           | ⚡ 10 张          | ✅ 无限制           |
| 多标签页提取                       | ⚡ 3 次/月        | ✅ 跨标签页         |
| 反向图片搜索                       | ⚡ 2 个引擎       | ✅ 4 个引擎         |
...
| **月付**   |  $3.99 | 每月                | 想短期试用 Pro 功能                 |
| **年付**   | $29.99 | 每年（约 37% 优惠） | 长期用户 —— 性价比最高              |
| **终身版** | $49.99 | 一次性付费          | 一次买断，永久使用 —— 无订阅烦恼    |
...
| 隐私优先（无分析追踪）                   | ✅ 支持               | n/a（无维护）           | ⚠️ 含广告网络     | ✅ 支持      |
...
| UI       | Preact + HTML/CSS                    | 轻量响应式组件，极小包体积   |
...
- **仅匿名使用统计** —— 我们仅收集匿名功能使用事件计数（不含网址、图片或个人数据）以改进产品；图片、页面内容和个人数据绝不会离开你的设备
- **无第三方追踪** —— 不接入 Google Analytics、不接入 Mixpanel、无广告网络；遥测仅上报到我们自己的服务端
- **始终在你的掌控之中** —— 卸载插件会清空所有本地数据
...
**最新版本**：`v1.1.1` —— 🐛 右键「提取图片 / 提取链接图片」在侧边栏关闭时不再无反应（现在右键即自动弹出面板）；解析/右键注入的图片按所属标签页正确分组；链接候选横幅切换标签页后不再消失。🔄 链接解析支持登录墙/内网站点（cookie 限定同站），每条链接解析后显示成功/失败状态点。完整列表见 [CHANGELOG.md](./CHANGELOG.md)。
