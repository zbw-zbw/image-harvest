...
**最新版本**：`v1.1.2` —— 📊 设置页「使用配额」新增链接解析配额行（免费版每月 3 次，实时剩余计数，用完置灰）；🔔 免费用户每次「解析原图」成功后 toast 显示本月剩余次数，最后一次直接说明 Pro 无限解析（软转化，不打断操作）；🌍 15 种语言全部同步。完整列表见 [CHANGELOG.md](./CHANGELOG.md)。
