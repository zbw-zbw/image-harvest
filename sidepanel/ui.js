...
function updateFilterButtonLabels() {
  // Size button
  const sizeBtn = document.querySelector('.filter-btn[data-filter="size"]');
  if (sizeBtn) {
    const hasSizeFilter = activeFilters.size !== 'all' || activeFilters.sizeMin > 0 || activeFilters.sizeMax !== Infinity;
    let label = filterLabelDefaults.size;
    if (activeFilters.size !== 'all') {
      const sizeLabels = { small: 'Small', medium: 'Medium', large: 'Large', xl: 'XL', custom: 'Custom' };
      label = sizeLabels[activeFilters.size] || label;
    }
  ...
  // Type button
  const typeBtn = document.querySelector('.filter-btn[data-filter="type"]');
  if (typeBtn) {
    const hasTypeFilter = activeFilters.types.length > 0;
    let label = filterLabelDefaults.types;
    if (hasTypeFilter) {
      const typeLabels = { png: 'PNG', jpg: 'JPG', jpeg: 'JPEG', webp: 'WebP', gif: 'GIF', svg: 'SVG', ico: 'ICO', bmp: 'BMP' };
      label = activeFilters.types.map(t => typeLabels[t] || t).join(', ');
    }
  ...
  // Layout button
  const layoutBtn = document.querySelector('.filter-btn[data-filter="layout"]');
  if (layoutBtn) {
    const hasLayoutFilter = activeFilters.layout !== 'all';
    let label = filterLabelDefaults.layout;
    if (hasLayoutFilter) {
      const layoutLabels = { square: 'Square', landscape: 'Landscape', portrait: 'Portrait', panorama: 'Panorama' };
      label = layoutLabels[activeFilters.layout] || label;
    }
    layoutBtn.textContent = label + '▾';
    layoutBtn.classList.toggle('active', activeFilters.layout !== 'all');
  }

  // URL button
  const urlBtn = document.querySelector('.filter-btn[data-filter="url"]');
  if (urlBtn) {
    const hasUrlFilter = !!activeFilters.urlKeyword;
    urlBtn.textContent = hasUrlFilter ? 'URL▾' : 'URL▾';
    urlBtn.classList.toggle('active', hasUrlFilter);
  }

  // Color button
  const colorBtn = document.querySelector('.filter-btn[data-filter="color"]');
  if (colorBtn) {
    const hasColorFilter = !!activeFilters.color;
  ...
  const groupBtn = document.querySelector('.filter-btn[data-filter="group"]');
  if (groupBtn) {
    groupBtn.classList.toggle('active', currentGroupMode !== 'none');
  }

  // Sort button (icon-only: toggle active state only)
  const sortBtn = document.querySelector('.filter-btn[data-filter="sort"]');
  if (sortBtn) {
    sortBtn.classList.toggle('active', currentSortMode !== 'size-desc');
  }
}
...
function checkNarrowMode() {
  ...
  if (elements.btnViewToggle) {
    ...
    const toolbarRight = elements.btnViewToggle.closest('.toolbar-right');
    if (toolbarRight) {
 ...
 * @param {string} options.message - Dialog description
...
// Progress Modal
// ============================================
let progressAbortCallback = null;

function showProgress(title, onAbort) {
...
function updateProgress(current, total, currentFile, imageCount) {
...
function showEmpty(isNoResults) {
  hideAll();
...
function calcSkeletonCount(containerHeight, isListView) {
...
function showLoading() {
  ...
  if (elements.imageGrid) {
    ...
    elements.imageGrid.style.visibility = '';
    ...
    scanSkeletonLimit = skeletonCount;
...
function resetStatusBar() {
  ...
  if (elements.btnDedup) elements.btnDedup.style.display = 'none';
