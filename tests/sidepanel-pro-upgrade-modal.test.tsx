...
// ──────────────────────────────────────────────────────────────────────────
// v1.2 IA redesign (CTA hero always present + key fold + 4-row compare)
// ──────────────────────────────────────────────────────────────────────────

describe('v1.2 IA redesign', () => {
  test('key input lives inside a collapsed <details> at the bottom', async () => {
    await openModal();
    const { ProUpgradeModal } = await import('../sidepanel/components/ProUpgradeModal');
    render(<ProUpgradeModal />);

    const details = document.querySelector('.pro-key-fold') as HTMLDetailsElement | null;
    expect(details).toBeTruthy();
    expect(details!.open).toBe(false);
    // The 4 license-ui ids moved INTO the fold — getElementById is
    // position-independent, so the activation contract still holds.
    ...
    expect(details!.querySelector('#link-pro-modal-get')).toBeTruthy();
  });

  test('pricing CTA stays visible when trial NOT eligible and becomes primary', async () => {
    (
    ...
    // trialEligible starts true (optimistic default) and flips after the
    // async eligibility check — wait for the trial CTA to disappear.
    await waitFor(() => {
      expect(document.querySelector('#btn-pro-modal-trial')).toBeNull();
    });
    // Pin: pre-v1.2 the whole CTA block vanished for ineligible users,
    // leaving the modal with NO conversion action at all.
    const pricing = document.querySelector('#btn-pro-modal-pricing');
    expect(pricing).toBeTruthy();
    expect(pricing!.className).toContain('btn-primary');
  });

  test('compare list shows exactly the 4 core rows (not 17)', async () => {
    await openModal();
    const { ProUpgradeModal } = await import('../sidepanel/components/ProUpgradeModal');
    render(<ProUpgradeModal />);
  ...
  test('no emoji icons remain in the feature list', async () => {
    await openModal();
    const { ProUpgradeModal } = await import('../sidepanel/components/ProUpgradeModal');
    render(<ProUpgradeModal />);
